...
class GXAdminAlert {
    constructor(awsRegion, srcEmailAddr = null, dstEmailAddr = null, adminAlertPhoneNumber) {
        ...
        if(isNullOrUndefined(adminAlertPhoneNumber)){
            if(isNullOrUndefined(process.env.AWS_ADMIN_ALERT_PHONE_NUMBER)){
                throw new Error(`Initializing ${thisFileName} require passing Admin Alert PhoneNumber argument or defining process.env.AWS_ADMIN_ALERT_PHONE_NUMBER`);
            }
            this.adminAlertPhoneNumber = process.env.AWS_ADMIN_ALERT_PHONE_NUMBER;
        }else{
            this.adminAlertPhoneNumber = adminAlertPhoneNumber;
        }
    }
    ...
    notify(subject, message, alert_type = GXADMIN_ALERT_TYPE.NORMAL){
        ...
        return new Promise( async function f(resolve, reject) {
            try {
                const result = await weakThis.sendMail(subject, message);
                if(alert_type === GXADMIN_ALERT_TYPE.URGENT){
                    await weakThis.sendSMS(subject);
                }
                resolve(result);
            }catch (e) {
    ...
    async sendSMS(subject) {
        ...
        var params = { Message: `MyGix Server error: ${subject}`, PhoneNumber: this.adminAlertPhoneNumber };
