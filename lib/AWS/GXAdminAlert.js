...
class GXAdminAlert {
    constructor(aws_region, srcEmailAddr = null, dstEmailAddr = null) {
    ...
    send_to_gx_logger(subject, msg){
        ...
        axios.post('http://54.213.16.81:3000/insert', {
            ...
            .then(function (response) {
                console.log(`${this_fileName}:> Succeeded. Response:${CircularJSON.stringify(response)}`)
            })
            .catch(function (error) {
                console.log(`${this_fileName}:> failed. error: ${CircularJSON.stringify(error)}`);
            });
    }
        ...
        return new Promise(function (resolve, reject) {
            ...
            try{
                ...
                new AWS.SES({apiVersion: '2010-12-01'}).sendEmail(params, function (err, data) {
                    if(err){
                        console.error(`${this_fileName}::send_mail:> failed on AWS.SES. error: ${CircularJSON.stringify(err)}`);
                        reject(err);
                        return
                    }

                    console.log(`${this_fileName}::send_mail:> succeeded. data.MessageId:${data.MessageId}`);
                    resolve(data);
                });
                ...
                console.error(`${this_fileName}::send_mail:> catched an error. data.MessageId:${e}`);
                reject(e);
            }
    ...
    async sendSMS(subject) {
        ...
        publishTextPromise.then(
            function (data) { console.log(`${this_fileName}::sendSMS > A SMS Message with ID:'${data.MessageId} was sent.'`); }).catch(
