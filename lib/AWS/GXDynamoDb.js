...
class GXAWSDynamoDb {
    ...
    /// var filtersWithComparatorJson = {
    //             "expirationDate" : {
    //                 "value":1234,
    //                 "comp": "="
    //             }
    //         }
    async scanWithComparator(tableName, filtersWithComparatorJson = undefined, indexName = undefined, nextToken = undefined){
        const weak_this = this;
        return new Promise(function(resolve, reject) {
            var params = { TableName: tableName };

            if(!isNullOrUndefined(filtersWithComparatorJson)){
                var FilterExpression = "";
                var ExpressionAttributeNames = {};
                var ExpressionAttributeValues = {};
                var count = 0;
                for (var key in filtersWithComparatorJson) {
                    ExpressionAttributeNames[`#${key}`] = key;
                    ExpressionAttributeValues[`:${key}`] = AWS.DynamoDB.Converter.input(filtersWithComparatorJson[key].value);
                    if (count === 0) { FilterExpression = `${FilterExpression} #${key} ${filtersWithComparatorJson[key].comp} :${key}`; }
                    else { FilterExpression = `${FilterExpression} AND #${key} ${filtersWithComparatorJson[key].comp} :${key}`; }
                    count += 1;
                }
                params.ExpressionAttributeNames = ExpressionAttributeNames;
                params.ExpressionAttributeValues = ExpressionAttributeValues;
