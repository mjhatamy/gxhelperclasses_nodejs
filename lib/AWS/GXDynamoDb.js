...
class GXAWSDynamoDb {
    ...
    async scan(tableName, filtersJson){
        ...
        return new Promise(function(resolve, reject) {
            ...
            for (var key in filtersJson) {
                ...
                if (count === 0) { FilterExpression = `${UpdateExpression} #${key} = :${key}`; }
                else { FilterExpression = `${UpdateExpression}, #${key} = :${key}`; }
                count += 1;
            }
            ...
            weak_this.dynamodb.scan(tableName, params, function (err, data) {
